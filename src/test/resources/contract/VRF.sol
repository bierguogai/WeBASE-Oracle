...
//      );
  ...
  function randomValueFromVRFProof(bytes memory proof)
    ...
    event TestRandom(
      uint256[2] pk, uint256[2] gamma, uint256[3] cSSeed, address uWitness,
      uint256[2] cGammaWitness, uint256[2] sHashWitness, uint256 zInv);

    function testRandomValueFromVRFProof(bytes memory proof)
    public  returns (uint256 output) {
      ...
      (pk, gamma, cSSeed, uWitness, cGammaWitness, sHashWitness, zInv) = abi.decode(
        proof, (uint256[2], uint256[2], uint256[3], address, uint256[2],
                uint256[2], uint256));
      emit TestRandom(pk, gamma, cSSeed, uWitness, cGammaWitness, sHashWitness, zInv);
//      verifyVRFProof(
//        pk,
//        gamma,
//        cSSeed[0], // c
